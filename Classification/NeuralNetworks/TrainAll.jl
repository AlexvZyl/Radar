    ...
    for f in frames
        for m in models
            ...
            elseif m == LeNet5StandardTemporal
                train(m, split=train_test_split, batchsize=batchsize, epochs=epochs, persons=p, frames_folder=frames_folder, timeout=timeout, λ=LeNet5StandardTemporal_weight, η=LeNet5StandardTemporal_learning_rate, dropout=dropout)
            end
        end
    end
end
